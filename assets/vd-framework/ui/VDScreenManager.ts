...
export default class VDScreenManager extends cc.Component {
  ...
  public static get instance(): VDScreenManager {
    if (!VDScreenManager._instance) {
      VDScreenManager._instance = cc.director
        .getScene()!
        .getComponentInChildren(VDScreenManager)!;
    }
  ...
  onLoad() {
    ...
    VDEventListener.on(
      VDEventListenerName.REMOVE_BASE_POPUP,
      this.removePopup,
      this
    );
    VDEventListener.on(
      VDEventListenerName.RESIZE_WINDOW_EVENT,
      this.resizeWindowAdapt,
      this
    );
  }
  ...
  resizeWindowAdapt() {
    ...
    const designRatio =
      (cc.view.getDesignResolutionSize().width * 1.0) /
      cc.view.getDesignResolutionSize().height;
    if (windowRatio > designRatio) {
      let cWidth = Math.floor(
        windowRatio * cc.view.getDesignResolutionSize().height
      );
      ...
      let cHeight = Math.floor(
        cc.view.getDesignResolutionSize().width / windowRatio
      );
      uiTransform.setContentSize(
  ...
  setupCommon() {
    console.log("SET UP COMMON");
    cc.resources.load(
      `images/bgr/bg_popup/spriteFrame`,
      cc.SpriteFrame,
      (error, spriteFrame) => {
    ...
    if (this.layerBackground) {
      // only for native app
      cc.resources.load(
        `images/bgr/default_sprite_splash/spriteFrame`,
        cc.SpriteFrame,
        (error, spriteFrame) => {
          // cc.log(`images/bg_popup`);
          if (spriteFrame) {
  ...
  public changeCanvasBackground(bgrSprite: cc.SpriteFrame) {
    if (bgrSprite && VDScreenManager.instance.layerBackground) {
      ...
      sprite
        ...
        .setContentSize(uiTransform.width, uiTransform.height);
  ...
  public createLayerGameBackground() {
    ...
    if (windowRatio > designRatio) {
      ...
      cc.log(
        `_setupLayerBackground 1: ${cWidth}, ${
          cc.view.getDesignResolutionSize().height
        }`
      );
    } else {
  ...
  private _registerEvent() {
    ...
    this.popupBackground.on(
      cc.Button.EventType.CLICK,
      this.hidePopupOnBackgroundTouchIfNeed,
      this
    );
  }
  ...
  public popToScreenAtIndex(
    ...
    this.scheduleOnce(function () {
      ...
      if (self._screenStack.length > screenIdx + 1) {
        ...
        self._screenStack.splice(
          screenIdx + 1,
          self._screenStack.length - screenIdx - 1
        );
      }
    });
    nextScreen.node.setOpacity(255);
    ...
    if (this._popUpStack.length > 0) {
            ...
            { opacity: 0 },
            {
              ...
              onUpdate: (target?: object, ratio?: number) => {
                popup.node.walk((child) => {
                  let spine: cc.sp.Skeleton = child.getComponent(
                    cc.sp.Skeleton
                  );
                  if (spine) {
      ...
      if (this._popUpStack.length > 0) {
        ...
        if (nextPopup.name === VD_POPUP_NOTIFY_NAME) {
          ...
          this.popupBackground.setZIndex(
            VD_ZINDEX.POPUP + this._popUpStack.length * 2
          );
          nextPopup.node.setZIndex(
            VD_ZINDEX.POPUP + this._popUpStack.length * 2 + 1
          );
        }
      } else {
  ...
  showPopupFromPrefabName(
    ...
    if (prefab) {
      this.showPopupFromPrefab(
        prefab,
        callback,
        hideWhenTouchOnBackground,
        animated,
        showBackgroundLayer
      );
    } else {
  ...
  showPopupFromNode(
    ...
    if (this._popUpStack.length == 0) {
      ...
      if (
        prevPopup.name === VD_POPUP_NOTIFY_NAME &&
        nodePopup.name === VD_POPUP_NOTIFY_NAME
      ) {
    ...
    if (this.popupBackground.active) {
      this.popupBackground.getComponent(cc.Sprite)!.enabled =
        showBackgroundLayer;
    }
  ...
  loadScreen(screen: cc.Prefab, callback: Function) {
    if (screen) {
      ...
      if (!comp) comp = node.addComponent(VDBaseScreen);
  ...
  showEffect(
    ...
    completedCallback: VoidFunction | null = null
  ) {
    ...
    if (animationName.length) {
      VDUtils.playAnimation(fxNode, animationName, false, () => {
        ...
        fxNode.destroy();
        this.layerEffect.children.length == 0 &&
          (this.layerEffect.active = false);
      });
    }
  }
